...
public class SyncUid {
  private static final Log LOG = LogFactory.getLog(SyncList.class);
  
  private static Options options = new Options();
  private static CommandLine cmdLine;
  
  private String thriftServers = Constants.BJ_HBASE_THRIFT_SERVERS;
  ...
  public int runTool(Configuration conf, String[] args) throws Exception {
    ...
    if (cmdLine.hasOption('l')) {
      ...
      oldRowKey = uid.startsWith("a") ? ("a" + uid) : uid;
    } else {
    ...
    conf.set(Constants.HBASE_ZOOKEEPER_QUORUM,
        Constants.BJ_HBASE_ZOOKEEPER_QUORUM);
    ...
    try {
      ...
      LOG.warn("", e);
      System.out.println("get old key: " + oldRowKey + " failed!");
    }
    ...
    for (Map.Entry<?,?> entry : familyMap.entrySet()) {
      ...
      LOG.info("get old personal cell, row: " + oldRowKey + ", column: "
          + PersonalUtil.OLD_FAMILY_STR + ":" + oldQualify + ", modify time: "
...
//        newColumn = pp.newColumn;
        ...
        if (cellList.isEmpty() || cellList.get(0).timestamp < oldTimestamp) {
          ...
          client.mutateRowTs(newTableByte, newRowByte, mutations, oldTimestamp,
              attributes);
        } else {
  ...
  public static void main(String[] args) throws Exception {
    ...
    SyncUid syncUid = new SyncUid();
    syncUid.runTool(conf, args);
  }
}
